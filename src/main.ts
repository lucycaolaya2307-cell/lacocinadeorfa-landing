...
(() => {
  ...
  try {
    console.log(
      '%c¡Alto!',
      'color: #b03c1c; font-size: 48px; font-weight: 700; text-shadow: 2px 2px 0 #3b130e;'
    );
    console.log(
      '%cEsta es una función del navegador pensada para desarrolladores. Si alguien te pidió copiar y pegar algo aquí, es muy probable que sea una estafa para tomar control de tu cuenta o información.',
      'color: #3b130e; font-size: 14px; line-height: 1.5;'
    );
    console.log(
      '%cSi no entiendes a fondo lo que haces aquí, cierra esta ventana.',
      'color: #6b2410; font-size: 13px; font-style: italic;'
    );
  } catch {
  ...
  const byId = <T extends HTMLElement>(id: string): T => {
    ...
    if (!el) throw new Error(`[main] Elemento #${id} no encontrado en el DOM`);
    return el as T;
  };
  ...
  const mobileMenu = byId<HTMLElement>('mobileMenu');
  const closeMenu = byId<HTMLButtonElement>('closeMenu');
  burger.addEventListener('click', () => mobileMenu.classList.add('open'));
  closeMenu.addEventListener('click', () => mobileMenu.classList.remove('open'));
  document.querySelectorAll<HTMLElement>('[data-close]').forEach(a =>
    a.addEventListener('click', () => mobileMenu.classList.remove('open'))
  );

  // ---------- 3. MENU FILTERING ----------
  const tabs = document.querySelectorAll<HTMLElement>('.menu__tab');
  const dishes = document.querySelectorAll<HTMLElement>('.dish');
  tabs.forEach(tab => {
  ...
  // ---------- 5. YEAR ----------
  byId<HTMLElement>('year').textContent = String(new Date().getFullYear());
})();
